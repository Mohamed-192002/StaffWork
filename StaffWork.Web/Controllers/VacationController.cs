...
namespace StaffWork.Web.Controllers
{
    public class VacationController : ApiBaseController<Vacation>
    {
        ...
        [HttpGet]
        public IActionResult NotStartVacation()
        {
            return View();
        }
        [HttpGet]
        ...
        public async Task<IActionResult> Create(VacationFormViewModel viewModel)
        {
            ...
            if (viewModel.StartDate != null)
            {
                ...
                {
                    Vacation.EndDate = viewModel.StartDate.Value.AddDays(Vacation.VacationDays);
                }
                else if (viewModel.VacationDuration == VacationDuration.Month)
                {
                    Vacation.EndDate = viewModel.StartDate.Value.AddMonths(Vacation.VacationDays);
                }
                else if (viewModel.VacationDuration == VacationDuration.Year)
                {
                    Vacation.EndDate = viewModel.StartDate.Value.AddYears(Vacation.VacationDays);
                }
                ////////////////
                if (!viewModel.IsAutoNotifi)
                {
                    ...
                    {
                        Vacation.CustomNotifiDate = Vacation.EndDate.Value.AddDays(-Vacation.CustomNotifiBeforeDays ?? 0);
                    }
                    else if (viewModel.CustomNotifiDuration == VacationDuration.Month)
                    {
                        Vacation.CustomNotifiDate = Vacation.EndDate.Value.AddMonths(-Vacation.CustomNotifiBeforeDays ?? 0);
                    }
                    else if (viewModel.CustomNotifiDuration == VacationDuration.Year)
                    {
                        Vacation.CustomNotifiDate = Vacation.EndDate.Value.AddYears(-Vacation.CustomNotifiBeforeDays ?? 0);
                    }
            ...
            if (viewModel.StartDate != null)
            {
                ...
                if (viewModel.IsAutoNotifi)
                {
                    var scheduleTime = vacation.EndDate.Value.AddDays(-1) - DateTime.Now;
                    if (scheduleTime.TotalSeconds > 0) // Ensure scheduling in the future
                    {
                        vacation.JobId = BackgroundJob.Schedule<NotifiJob>(
                    ...
                    else
                    {
                        ...
                        vacation.JobId = BackgroundJob.Enqueue<NotifiJob>(x => x.ScheduleNotifiJob(vacation));
                    }
                }
                else
                {
        ...
        public async Task<IActionResult> EditAsync(VacationFormViewModel viewModel)
        {
            ...
            if (viewModel.StartDate != null)
            {
                #region Send Notification
                var vacation = await BussinesService.GetAsync(x => x.Id == Vacation.Id, ["Employee"]);
                if (vacation.JobId != null)
                {
                    BackgroundJob.Delete(vacation.JobId);
                }
                if (viewModel.IsAutoNotifi)
                {
                    var scheduleTime = vacation.EndDate.Value.AddDays(-1) - DateTime.Now;
        ...
        public async Task<IActionResult> GetVacations()
        {
            ...
            VacationQuery = (IQueryable<Vacation>)await BussinesService.GetAllAsync(x => !x.IsReturned && x.StartDate != null
                            , ["Employee", "VacationType"], orderBy: x => x.EndDate);
        ...
        public async Task<IActionResult> GetReturnedVacations()
        {
            ...
            IQueryable<Vacation> VacationQuery;
            VacationQuery = (IQueryable<Vacation>)await BussinesService.GetAllAsync(x => x.IsReturned
        ...
        [HttpPost]
        public async Task<IActionResult> GetNotStartVacations()
        {
            var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
            if (string.IsNullOrEmpty(userId))
                return Unauthorized();
            ...
            IQueryable<Vacation> VacationQuery;
            VacationQuery = (IQueryable<Vacation>)await BussinesService.GetAllAsync(x => x.StartDate == null
            , ["Employee", "VacationType"]);
        ...
        public async Task<IActionResult> NotStartVacationExportToExcelAsync(string searchValue, string sortColumn, string sortColumnDirection)
        {
            IQueryable<Vacation> VacationQuery;
            VacationQuery = (IQueryable<Vacation>)await BussinesService.GetAllAsync(x => x.StartDate == null
            , ["Employee", "VacationType"]);
